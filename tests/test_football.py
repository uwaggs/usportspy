...
def test_fb_get_offence():
    ...
    df = fb_get_offence(gender="m", seasons=[2023])
    assert isinstance(df, pd.DataFrame)
    assert len(df) > 0
...
def test_fb_get_defence():
    ...
    df = fb_get_defence(gender="m", seasons=[2023])
    assert isinstance(df, pd.DataFrame)
    assert len(df) > 0
...
def test_fb_get_kicking():
    ...
    df = fb_get_kicking(gender="m", seasons=[2023])
    assert isinstance(df, pd.DataFrame)
    assert len(df) > 0
...
def test_fb_get_returns():
    ...
    df = fb_get_returns(gender="m", seasons=[2023])
    assert isinstance(df, pd.DataFrame)
    assert len(df) > 0
...
def test_fb_get_pbp():
    ...
    df = fb_get_pbp(gender="m", seasons=[2023])
    assert isinstance(df, pd.DataFrame)
    assert len(df) > 0
...
def test_fb_get_scoring_summaries():
    ...
    df = fb_get_scoring_summaries(gender="m", seasons=[2023])
    assert isinstance(df, pd.DataFrame)
    assert len(df) > 0
