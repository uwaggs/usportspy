...
def test_volleyball_get_team_box_score():
    ...
    df = volleyball_get_team_box_score(gender="m", seasons=[2024])
    assert isinstance(df, pd.DataFrame)
    assert len(df) > 0
...
def test_volleyball_get_player_box_score():
    ...
    df = volleyball_get_player_box_score(gender="w", seasons=[2024])
    assert isinstance(df, pd.DataFrame)
    assert len(df) > 0
...
def test_volleyball_get_pbp():
    ...
    df = volleyball_get_pbp(gender="m", seasons=[2024])
    assert isinstance(df, pd.DataFrame)
    assert len(df) > 0
